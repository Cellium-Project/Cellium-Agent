...
| **多平台接入** | 支持 WebUI、QQ、Telegram、飞书、微信等多平台对话 |
...
| 三层记忆系统 | 人格记忆 + 会话记忆 + 长期记忆（FTS5全文检索 + 向量检索混合召回） |
...
| 多通道接入 | 支持 QQ、Telegram、飞书、微信等外部平台，通过 ChannelManager 统一管理消息路由、文件传输与注入 |
...
<div align="center">
  <img src="https://agent.cellium.top/github-assets/demo.gif" alt="快速运行演示" width="600">
...
| ThreeLayerMemory | 三层记忆：人格 + 会话 + FTS5 + 可选向量检索 |
...
长期记忆支持可选的 API 向量嵌入，用于语义搜索：

- **维度**：可配置（默认 1536，text-embedding-3-small）
- **提供商**：任意 OpenAI 兼容的 Embedding API
- **相似度**：余弦相似度（cosine similarity）
- **混合召回**：FTS5 全文检索 + 向量相似度融合排序
- **配置**：`config/agent/embedding.yaml` — 启用/禁用、API Key、模型、服务地址
- **按需使用**：默认关闭，不依赖外部服务
...
任务执行结果会自动推送到创建任务的平台（WebUI/QQ/Telegram/飞书/微信）。
...
事件触发结果会自动推送到对应的 session（WebUI/QQ/Telegram/飞书/微信）。
