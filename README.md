...
长期记忆采用分层分类设计，支持 Schema 类型和 Category 分类两个维度：

| Schema 类型 | 用途 | Category 分类 | 说明 |
|-------------|------|---------------|------|
| `general` | 通用记忆 | `general` | 日常对话、知识问答 |
| `general` | 会话笔记 | `user_info` | 会话目标、历史目标 (goal/goal_history) |
| `general` | 会话笔记 | `command` | 已完成的操作命令 (completed) |
| `general` | 会话笔记 | `project` | 关键发现 (finding) |
| `profile` | 用户画像 | `preference` | 用户偏好、个人信息 |
| `project` | 项目相关 | `project` | 项目配置、代码仓库 |
| `issue` | 问题排查 | `troubleshooting` | 故障记录、解决方案、错误记录 (error) |
| `issue` | 问题排查 | `code` | 代码相关的发现和操作 |

**会话压缩自动提取**：

会话压缩时自动提取以下类型的笔记，并映射到相应分类存入长期记忆：
