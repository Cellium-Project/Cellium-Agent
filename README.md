...
| 组件沙箱安全 | 四层防护：AST静态检查 + 进程隔离 + 路径透明映射 + 危险方法拦截 |
...
Agent 编写的组件代码在独立的沙箱环境中运行，采用四层安全防护：

| 层级 | 机制 | 作用 |
|------|------|------|
| **AST 静态检查** | 编译前扫描危险导入 | 拦截 eval/exec/__import__ 等动态代码执行 |
| **进程隔离** | 组件在独立子进程运行 | 崩溃不影响主程序，通过 Queue 通信 |
| **路径透明映射** | 所有文件操作映射到 sandbox_root | C:/Windows → sandbox_root/C_/Windows，防路径穿越 |
| **危险方法拦截** | 运行时拦截 os.system/subprocess 等 | 禁止执行系统命令，保护宿主机安全 |

组件可以正常使用 `open()`、`os.listdir()` 等文件操作，但所有路径都会被透明映射到项目目录下的 `sandbox_root/` 文件夹中，无法访问真实系统文件。同时 `os.system()`、`subprocess.run()` 等危险方法会被运行时拦截。
