...
class FileTool(BaseTool):
    ...
    name = "file"
    description = (
        "文件操作工具。通过 command 字段选择子命令，各命令的 mode 互不相同！\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "【重要】mode 参数是命令专用的，不同命令支持不同的 mode：\n"
        "  • read 命令的 mode: full | context | summary | compact\n"
        "  • edit 命令的 mode: replace | replace_all | insert | append | regex | range | delete\n"
        "  • insight 命令的 mode: grep | structure | symbol | files\n"
        "  ⚠️ 绝不要混用！例如：read 不支持 range，edit 不支持 full\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "【read】读取文件内容（只读操作）\n"
        "  command=read, path=文件路径\n"
        "  mode=full: 完整读取，可用 offset/limit 分页\n"
        "  mode=context: 读取目标附近内容，需提供 target 参数\n"
        "  mode=summary: 提取符号签名（类/函数定义）\n"
        "  mode=compact: 压缩读取（折叠 imports/docstrings）\n\n"
        "【edit】修改文件内容（写入操作，自动验证+回滚）\n"
        "  command=edit, path=文件路径\n"
        "  mode=replace: 替换文本（需 old_text + new_text）\n"
        "  mode=replace_all: 替换所有匹配\n"
        "  mode=insert: 在指定行号前插入（需 line + content）\n"
        "  mode=append: 追加内容到文件末尾（需 content）\n"
        "  mode=regex: 正则替换（需 pattern + replacement）\n"
        "  mode=range: 按行号范围替换（需 start_line + end_line + new_text）\n"
        "  mode=delete: 删除行范围（需 start_line + end_line）\n\n"
        "【insight】探索工程结构（不知道目标位置时使用）\n"
        "  command=insight\n"
        "  mode=grep: 搜索内容（需 query, 可选 pattern 过滤文件）\n"
        "  mode=structure: 获取文件/目录结构大纲\n"
        "  mode=symbol: 搜索符号定义（需 query）\n"
        "  mode=files: 搜索文件名（可选 pattern）\n\n"
        "【fs】文件系统操作\n"
        "  command=fs, action=list|mkdir|delete|exists|create\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "典型流程: insight(找位置) → read(读内容) → edit(改内容)\n"
        "读取大文件时: read(mode=full, offset=0, limit=100) 分页读取"
    )
